---
name: autopilot
description: Run an unattended multi-hour build session against this site's shared backlog — research, queue, implement, verify, deploy, repeat. Use when starting a long session that should continue without supervision.
---

# Autopilot

An unattended session. Nobody is watching, which changes two things: you decide
everything you are entitled to decide, and you never report a result you did not
observe.

Full reasoning and rationale: `docs/15-autopilot.md`. This is the operating copy.

## Where the backlog lives

Outside every repo, so several sites in several VSCode workspaces share one tool
...
0. **Check for new instructions.** If `plan.md` (or `plan.json`) exists in the
   project root, re-import it:
   `backlog import <site> --from plan.md`. Duplicate titles are skipped, so this
   is safe every cycle and costs nothing. This is how Cowork hands you work
   mid-session without anybody relaying it — it revises the plan, you pick the
...
4. **Verify.** Run the verify command. It must pass, and check its **own** exit
   code — piping it through anything else means `&&` tests the pipe, not the gate.
   That mistake shipped a failing commit once already. If a gate fails, fix the
...
with evidence. In rough order of what has actually paid off here:
...
- **Fix what is measurably broken.** Run the app as a user would and look at the
  ...
  tripped an assertion, because assertions ask "is the text present" and it was —
  at height zero.
...
Genuinely only these. Use `backlog block` and move to the next item — do not stall
a whole session waiting for a reply that is coming in the evening.

- **Credentials and logged-in sessions** — anything needing an account he owns.
- **Money** — payments, paid listings, anything with a price.
- **Public, irreversible acts under his name** — publishing to a store, posting to
  a community, emailing anyone, changing repository visibility. Redeploying an
...
- **A regression test that has not been observed to fail is a comment.** When you
  ...
  repo passed against the very bug they were written for (D-91, D-93).
