...
test.describe('privacy — nothing is ever uploaded', () => {
  test('a full conversion sends zero bytes anywhere', async ({ page, baseURL }) => {
    ...
    const foreign = requests.filter((r) => !isAllowedOrigin(r.url, baseURL ?? ''));
    expect(foreign, 'cross-origin requests: ' + JSON.stringify(foreign)).toEqual([]);
  });
  ...
  test('no beacon fires on unload', async ({ page, baseURL }) => {
    ...
    const onUnload = requests.slice(before).filter((r) => !r.url.startsWith('about:'));
    expect(onUnload, 'requests on unload: ' + JSON.stringify(onUnload)).toEqual([]);
    expect(requests.every((r) => !r.hasBody)).toBe(true);
    expect(requests.filter((r) => !isAllowedOrigin(r.url, baseURL ?? '')).length).toBe(0);
  });
});
