...
# Step 1 — binary search, narrowing from the known-too-big top end.
while scale >= minScale:
    # maxQuality was probed only at scale 1, so exclude it only there
    hiBound = (maxQuality - 1) if scale == 1 else maxQuality
    lo = minQuality, hi = hiBound, best = null
